...
genDodaj c inp out 0 = p1 ++ inp ++ (show (c+lp1-1) ++ "R") ++ "0#" ++ out ++  (show (c + 1)++"L") ++ ",\n"
    where p1 = genP1 c (show (c+lp1-1) ++ "R") (show (c+lp1-1) ++"R") 0
          lp1 = length $ lines $ genP1 0 "" "" 0
genDodaj c inp out k = p1 ++ inp ++ (show (c+lp1 -2) ++ "L") ++ "0#" ++ out ++ (show (c +1)++"L") ++ ",\n"
    where p1 = genP1 c (show (c+lp1-1) ++ "R") (show (c+lp1-1) ++"R") k
